...
class BingGeocoderTest < BaseGeocoderTest #:nodoc: all
  ...
  def setup 
    super
    @bing_full_hash = {:street_address=>"100 Spear St", :city=>"San Francisco", :state=>"CA", :zip=>"94105", :country_code=>"US"}
    @bing_city_hash = {:city=>"San Francisco", :state=>"CA"}
    @bing_full_loc = Geokit::GeoLoc.new(@bing_full_hash)
    @bing_city_loc = Geokit::GeoLoc.new(@bing_city_hash)
  end  
  ...
  def test_bing_full_address
    ...
    assert_equal "37.792156,-122.394012", result.ll # slightly dif from yahoo
    assert_equal "100 Spear St, San Francisco, CA 94105-1522", result.full_address #slightly different from yahoo
  ...
  def test_service_unavailable
    ...
    url = "http://dev.virtualearth.net/REST/v1/Locations/#{Geokit::Inflector::url_escape(@address)}?key=#{Geokit::Geocoders::bing}&o=xml"
    Geokit::Geocoders::BingGeocoder.expects(:call_geocoder_service).with(url).returns(response)
    assert !Geokit::Geocoders::BingGeocoder.geocode(@bing_city_loc).success
  end 
